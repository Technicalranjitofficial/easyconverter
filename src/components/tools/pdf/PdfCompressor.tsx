...
import { compressPdf, type CompressionLevel } from "@/lib/converters/pdfConverter";
...
const LEVELS: { value: CompressionLevel; label: string; desc: string }[] = [
  { value: "screen", label: "Maximum",   desc: "Smallest file, lower quality images" },
  { value: "ebook",  label: "Balanced",  desc: "Good balance of size and quality" },
  { value: "print",  label: "High Quality", desc: "Best quality, moderate compression" },
];
  ...
  const handleCompress = async () => {
    ...
    await Promise.allSettled(ready.map(async item => {
      try {
        ...
        setItems(prev => prev.map(i => i.id === item.id ? { ...i, status: "error" as const, error: "Failed" } : i));
      }
    }));
    ...
    <div className="w-full space-y-5">
            ...
            <div className="grid grid-cols-3 gap-2 mb-4">
              {LEVELS.map(l => (
                <button key={l.value} onClick={() => setLevel(l.value)}
                  ...
                  <p className={`text-xs font-bold mb-0.5 ${level === l.value ? "text-indigo-700" : "text-slate-700"}`}>{l.label}</p>
      ...
      {!allDone && readyCount > 0 && (
        <button onClick={handleCompress} disabled={running}
          ...
          {running ? <><Loader2 className="w-5 h-5 animate-spin" />Compressing…</> : <><Minimize2 className="w-5 h-5" />Compress {readyCount} PDF{readyCount !== 1 ? "s" : ""}</>}
        </button>
      )}
        ...
        <div className="space-y-2 animate-slide-up">
              ...
              <div key={item.id} className="flex items-center gap-3 p-3 rounded-xl border-l-[3px] border-l-emerald-500
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <div className="flex items-center gap-2 mt-0.5">
                    ...
                    {saving > 0 && <span className="text-[10px] font-bold text-emerald-600 bg-emerald-50 px-1.5 py-0.5 rounded-full">-{saving}%</span>}
                  </div>
                </div>
          ...
          <button onClick={() => { setItems([]); setAllDone(false); }}
            className="w-full flex items-center justify-center gap-2 py-3 rounded-2xl text-sm font-semibold
                       text-slate-600 bg-slate-100 hover:bg-slate-200 transition-colors mt-2">
